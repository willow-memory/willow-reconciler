...
31. ✅ **shipped**: the fleet CI floor (fleet plan decision 5), in this repo's `tests.yml`: a Linux job whose Python matrix is derived from `pyproject.toml`'s `Programming Language :: Python :: 3.X` classifiers rather than typed twice; a Windows job on the floor and ceiling Pythons, because the hooks write an executable and the trailer parser reads `git log`, and neither has ever run where separators and line endings differ; a lint job with `ruff` pinned to an exact version running `ruff check` and `ruff format --check`, which lands the ruff half of item 21 (mypy stays open there); CodeQL over `python` and `actions`; and an aggregate `test` job that `needs:` every leg, runs `if: always()`, and fails on any result that is not `success` — a skipped or cancelled leg is a failure, said explicitly, because GitHub's default reads a skipped required check as passing. Guarded by a structural test that holds the matrix to the classifiers, the ruff pin to an exact version, and the gate to naming every job, each planted.
