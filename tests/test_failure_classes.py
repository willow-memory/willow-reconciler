...
def test_the_stdlib_missing_executable_message_is_still_classified():
    assert (
        classify("[Errno 2] No such file or directory: 'git'")
...
def test_permission_denied_is_unaffected_by_the_reorder():
    assert (
        classify("fatal: cannot change to '/x': Permission denied")
...
def test_the_resolved_path_never_reaches_the_output():
    hostile = (
        "[Errno 2] No such file or directory: '/home/sean-campbell/work/acme/README.md'"
    )
