...
def _workflow_packaged(workflow_text: str) -> tuple[str, ...]:
    ...
    return tuple(
        part.strip().strip("'\"") for part in m.group(1).split(",") if part.strip()
    )
...
def _pyproject_packaged(pyproject_text: str) -> tuple[str, ...]:
    ...
    assert m, (
        "pyproject.toml must declare [tool.hatch.build.targets.wheel] packages = [...]"
    )
    return tuple(
        part.strip().strip("'\"") for part in m.group(1).split(",") if part.strip()
    )
...
def test_the_guard_check_fires_on_a_planted_tree_that_arms_automerge_without_it(
    tmp_path,
):
    ...
    assert _pr_title_guard_offenders(_tree(tmp_path, arms=True, guard=False)) == [
        PR_TITLE_GUARD
    ]
...
def test_packaged_in_the_workflow_agrees_with_pyproject():
    ...
    packaged = _workflow_packaged(
        (REPO_ROOT / PR_TITLE_GUARD).read_text(encoding="utf-8")
    )
