...
def _resolve_repo(
    repo: str, fleet_root: Path | None = None
...
def _repo_not_found_error(repo: str, candidates: list[Path]) -> str:
    ...
    return (
        f"error: repo '{repo}' not found — no `.git` at any candidate tried: "
...
def _load(repo: str, doc: str, fleet_root: Path | None):
    ...
    try:
        ...
        print(
            f"error: could not read --doc {doc} under {repo}: {classify_file(str(e))}",
            file=sys.stderr,
        )
        return 2
    ...
    if not items:
        print(
            f"error: no numbered items found in {doc} — wrong file, or a doc with "
            f"no top-level `N. ` list items.",
            file=sys.stderr,
        )
        return 1
...
def run(
    ...
    verdicts = [
        classify_item(idea_id(item.num), item.num, item.text, gitlog) for item in items
    ]
    ledger = build_ledger(
        doc=doc,
        repo=repo,
...
def cmd_id(
    ...
    if len(matches) > 1:
        ...
        print(
            f"error: {selector} matches {len(matches)} items in {doc}; narrow it:",
            file=sys.stderr,
        )
        for i in matches:
...
def cmd_verify(
    repo: str, doc: str, fmt: str = "markdown", fleet_root: Path | None = None
) -> int:
    ...
    if fmt == "json":
        ...
        for row in result["unresolved"]:
            lines.append(
                f"- **UNRESOLVED** `{row['idea_id']}` in {row['short_sha']}: "
                f"{row['subject']}"
            )
        for row in result["resolved"]:
            lines.append(
                f"- ok `{row['idea_id']}` ({row['status']}) in "
...
def cmd_install_hook(
    repo: str, force: bool = False, fleet_root: Path | None = None
) -> int:
    repo_path, candidates = _resolve_repo(repo, fleet_root)
    if repo_path is None:
        print(_repo_not_found_error(repo, candidates), file=sys.stderr)
...
def cmd_benchmark(
    ...
    for d in result["detail"]:
        if not d["recovered"]:
            lines.append(
                f"  - [MISS] {d['idea_id']}: expected={d['expected']} got={d['got']}"
            )
        else:
            lines.append(
                f"  - [{d['provenance'].upper()}] {d['idea_id']}: "
...
def _render(ledger: dict, fmt: str) -> str:
    ...
    if ledger["duplicate_nums"]:
        lines.append(
            f"- **duplicate item numbers** (surfaced, not resolved): "
    ...
    for row in ledger["items"]:
        lines.append(
            f"- `{row['idea_id']}` **{row['status']}** (rule {row['rule']}, "
    ...
    if "validation" in ledger:
        ...
        for d in ho["detail"]:
            ...
            lines.append(
                f"  - [{mark}] {d['idea_id']}: expected={d['expected']} "
...
def _fleet_row_cells(row: dict) -> tuple[str, ...]:
    if row["doc_status"] != DOC_OK:
        return (f"doc: missing ({row['doc_error']})",) + ("—",) * (
            len(_FLEET_COLUMNS) - 1
        )
    return (
        str(row["items_parsed"]),
        str(row["dropped"]),
        str(row["explicit_landed"]),
        str(row["explicit_partial"]),
        str(row["inferred_landed"]),
        str(row["inferred_partial"]),
        str(row["none"]),
        str(row["independent_recovery_rate"]),
...
def main(argv=None) -> int:
    ...
    r = sub.add_parser(
        "run",
        help="parse a doc's numbered items and classify each "
    ...
    r.add_argument(
        "--doc", required=True, help="path to the doc, relative to --repo's root"
    )
    r.add_argument(
        "--format", default="markdown", choices=("markdown", "json"), dest="fmt"
    )
    r.add_argument(
        "--validate",
        action="store_true",
    ...
    i = sub.add_parser(
        "id",
        help="print the Idea-Id trailer line for one item, "
    ...
    sel.add_argument(
        "--grep",
        help="case-insensitive substring of the item text; "
    ...
    v = sub.add_parser(
        "verify",
        help="check that every Idea-Id trailer in the repo's "
    ...
    h.add_argument(
        "--force", action="store_true", help="replace a hook this tool did not write"
    )
    ...
    f = sub.add_parser(
        "fleet",
        help="reconcile the same doc across many repos and pool "
    ...
    c = sub.add_parser(
        "conventions",
        help="publish the fleet's convention set — the one "
